...
#define RINGBUFSZ_INIT (16*1024*1024)
...
void usage(void)
{
        printf("rtl_tcp, an I/Q spectrum ringbuffer-server for RTL2832 based DVB-T receivers - modified by Bas ON5HB for websdr.org\n\n"
                "Usage:\n"
                "\t[-a listen address]\n"
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "a:p:f:g:s:b:d:P:T:v")) != -1) {
		switch (opt) {
		case 'd':
